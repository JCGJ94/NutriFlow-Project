...
export function NutritionHeader({ kcal, protein, carbs, fat, title }: NutritionHeaderProps) {
  const { t } = useLanguage();
    ...
    <div className="bg-white dark:bg-surface-800 rounded-3xl border border-surface-100 dark:border-surface-700 p-6 md:p-8 shadow-sm transition-colors duration-300">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-3">
          <MacroBadge label={t('macros.calories')} value={kcal} unit="kcal" color="calories" size="lg" />
          <MacroBadge label={t('macros.protein')} value={protein} color="protein" size="lg" />
          <MacroBadge label={t('macros.carbs')} value={carbs} color="carbs" size="lg" />
          <MacroBadge label={t('macros.fats')} value={fat} color="fat" size="lg" />
        </div>
      </div>
